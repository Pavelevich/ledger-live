...
function loadCoinModal(key: CoinModalKey): Promise<CoinModalComponent> {
  if (!modalPromiseCache[key]) {
    modalPromiseCache[key] = annotate(coinModalImports[key]().then(m => m.default));
  }
  return modalPromiseCache[key]!;
}
...
export function preloadCoinModals(names: readonly CoinModalKey[]): void {
  for (const name of names) loadCoinModal(name).catch(() => {});
}
