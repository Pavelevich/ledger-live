...
describe("AccountBalanceSummaryFooter", () => {
  ...
  it("should return null for token accounts when token config flag is disabled", () => {
    ...
    const { container } = render(<AccountBalanceSummaryFooter account={tokenAccount} />, {
      initialState: { accounts: [ALEO_ACCOUNT_1, tokenAccount] },
    });
  ...
  it("should display transparent and private balances for token accounts when token config flag is enabled", () => {
    ...
    const tokenAccount = {
      ...
      transparentBalance: tokenBalance,
      privateBalance: null,
    } as unknown as TokenAccount;
    ...
    expect(screen.getByText("Available balance")).toBeInTheDocument();
    expect(screen.getByText("Transparent balance")).toBeInTheDocument();
    expect(screen.getAllByText(`formatted: ${tokenBalance}`).length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText(PRIVATE_BALANCE_PLACEHOLDER).length).toBeGreaterThanOrEqual(1);
