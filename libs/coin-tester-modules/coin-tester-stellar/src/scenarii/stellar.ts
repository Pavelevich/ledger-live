...
export const scenarioStellar: Scenario<GenericTransaction, Account> = {
  ...
  setup: async () => {
    ...
    const { currencyBridge, accountBridge } = await getBridges();
    ...
    await fundViaFriendbot(signer.address);
    await fundViaFriendbot(RECIPIENT_ADDRESS).catch(() => undefined);
    await fundViaFriendbot(ISSUER_ADDRESS).catch(() => undefined);
    ...
    const account = makeAccount(signer.address);
