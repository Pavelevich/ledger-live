...
function getNextPublicTransactionType({
  isTokenTx,
  isSelfTransfer,
}: {
  isTokenTx: boolean;
...
function getNextPrivateTransactionType({
  isTokenTx,
  isSelfTransfer,
}: {
  isTokenTx: boolean;
...
function preparePublicTransaction({
  ...
  nextTransactionType: AleoTransaction["mode"];
  estimatedFees: BigNumber;
  isSelfTransfer: boolean;
}): AleoTransaction {
...
function preparePrivateTransaction({
  ...
  account: AleoAccount;
  transaction: AleoTransactionPrivate;
  nextTransactionType: AleoTransactionPrivate["mode"];
  config: AleoCoinConfig;
  estimatedFees: BigNumber;
...
>["prepareTransaction"] = async (account, transaction) => {
  ...
  if (isPrivateTransaction(transaction)) {
    const nextTransactionType = getNextPrivateTransactionType({ isTokenTx, isSelfTransfer });
  ...
  const nextTransactionType = getNextPublicTransactionType({ isTokenTx, isSelfTransfer });
  const feeEstimation = estimateFees({
    configOrCurrencyId: config,
    transactionType: nextTransactionType,
  });
