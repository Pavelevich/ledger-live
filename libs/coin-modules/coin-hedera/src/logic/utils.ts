...
export function isNodeOverstaked(
  node: Pick<HederaMirrorNode, "stake" | "max_stake">,
): boolean | null {
  if (node.stake === null || node.max_stake === null) return null;
  return BigInt(node.stake) >= BigInt(node.max_stake);
}
